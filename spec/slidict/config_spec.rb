...
RSpec.describe Slidict::Config do
  ...
  describe "#merge" do
    it "overrides only the provided values" do
      config = described_class.from_env("SLIDICT_LLM_BASE_URL" => "http://localhost:11434/v1").merge(model: "gpt-4o")
    ...
    it "is not enabled without a base_url even if an api_key is set" do
      config = described_class.from_env("SLIDICT_LLM_API_KEY" => "key")

      expect(config.llm_enabled?).to be(false)
    end

    it "disables the LLM when enabled: false is given" do
      config = described_class.from_env("SLIDICT_LLM_BASE_URL" => "http://localhost:11434/v1").merge(enabled: false)
